...
nextchar:
	 ...
	 */
#ifdef	USE_INET6
	if (ipf_yyexpectaddr != 0 && isbuilding == 0 &&
	    (ishex(c) || isdigit(c) || c == ':')) {
...
buildipv6:
...
#ifdef	USE_INET6
		ipf_yystr = ipf_yytexttostr(0, ipf_yypos - 1);
		if (ipf_yystr != NULL) {
			char *s;

			for (s = ipf_yystr; *s && ishex(*s); s++)
				;
			if (!*s && *ipf_yystr) {
				isbuilding = 0;
				c = *ipf_yystr;
	...
	if (ipf_yydebug)
		printf("isbuilding %d ipf_yyvarnext %d nokey %d fixed %d addr %d\n",
		       isbuilding, ipf_yyvarnext, nokey, ipf_yydictfixed, ipf_yyexpectaddr);
	if (isbuilding == 1) {
		wordtab_t *w;
		...
		if ((ipf_yyvarnext == 0) && (nokey == 0)) {
			...
			if (w == NULL && ipf_yywordtab != NULL && !ipf_yydictfixed) {
				ipf_yyresetdict();
				w = ipf_yyfindkey(ipf_yystr);
			}
		} else
	...
	if (rval == YY_STR) {
		if (ipf_yysavedepth > 0 && !ipf_yydictfixed)
			ipf_yyresetdict();
		if (ipf_yyexpectaddr != 0)
			ipf_yyexpectaddr = 0;
	}
	...
	if (ipf_yydebug)
		printf("lexed(%s) %d,%d,%d [%d,%d,%d] => %d @%d\n",
		       ipf_yystr, isbuilding, ipf_yyexpectaddr, ipf_yysavedepth,
		       string_start, string_end, pos, rval, ipf_yysavedepth);
...
void ipf_yysetfixeddict(newdict)
	wordtab_t *newdict;
{
	if (ipf_yydebug)
		printf("ipf_yysetfixeddict(%lx)\n", (u_long)newdict);
