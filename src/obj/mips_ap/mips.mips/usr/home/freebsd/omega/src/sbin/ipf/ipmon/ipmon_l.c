...
nextchar:
	 ...
	 */
#ifdef	USE_INET6
	if (ipmon_yyexpectaddr != 0 && isbuilding == 0 &&
	    (ishex(c) || isdigit(c) || c == ':')) {
...
buildipv6:
...
#ifdef	USE_INET6
		ipmon_yystr = ipmon_yytexttostr(0, ipmon_yypos - 1);
		if (ipmon_yystr != NULL) {
			char *s;

			for (s = ipmon_yystr; *s && ishex(*s); s++)
				;
			if (!*s && *ipmon_yystr) {
				isbuilding = 0;
				c = *ipmon_yystr;
	...
	if (ipmon_yydebug)
		printf("isbuilding %d ipmon_yyvarnext %d nokey %d fixed %d addr %d\n",
		       isbuilding, ipmon_yyvarnext, nokey, ipmon_yydictfixed, ipmon_yyexpectaddr);
	if (isbuilding == 1) {
		wordtab_t *w;
		...
		if ((ipmon_yyvarnext == 0) && (nokey == 0)) {
			...
			if (w == NULL && ipmon_yywordtab != NULL && !ipmon_yydictfixed) {
				ipmon_yyresetdict();
				w = ipmon_yyfindkey(ipmon_yystr);
			}
		} else
	...
	if (rval == YY_STR) {
		if (ipmon_yysavedepth > 0 && !ipmon_yydictfixed)
			ipmon_yyresetdict();
		if (ipmon_yyexpectaddr != 0)
			ipmon_yyexpectaddr = 0;
	}
	...
	if (ipmon_yydebug)
		printf("lexed(%s) %d,%d,%d [%d,%d,%d] => %d @%d\n",
		       ipmon_yystr, isbuilding, ipmon_yyexpectaddr, ipmon_yysavedepth,
		       string_start, string_end, pos, rval, ipmon_yysavedepth);
...
void ipmon_yysetfixeddict(newdict)
	wordtab_t *newdict;
{
	if (ipmon_yydebug)
		printf("ipmon_yysetfixeddict(%lx)\n", (u_long)newdict);
