...
nextchar:
	 ...
	 */
#ifdef	USE_INET6
	if (ipnat_yyexpectaddr != 0 && isbuilding == 0 &&
	    (ishex(c) || isdigit(c) || c == ':')) {
...
buildipv6:
...
#ifdef	USE_INET6
		ipnat_yystr = ipnat_yytexttostr(0, ipnat_yypos - 1);
		if (ipnat_yystr != NULL) {
			char *s;

			for (s = ipnat_yystr; *s && ishex(*s); s++)
				;
			if (!*s && *ipnat_yystr) {
				isbuilding = 0;
				c = *ipnat_yystr;
	...
	if (ipnat_yydebug)
		printf("isbuilding %d ipnat_yyvarnext %d nokey %d fixed %d addr %d\n",
		       isbuilding, ipnat_yyvarnext, nokey, ipnat_yydictfixed, ipnat_yyexpectaddr);
	if (isbuilding == 1) {
		wordtab_t *w;
		...
		if ((ipnat_yyvarnext == 0) && (nokey == 0)) {
			...
			if (w == NULL && ipnat_yywordtab != NULL && !ipnat_yydictfixed) {
				ipnat_yyresetdict();
				w = ipnat_yyfindkey(ipnat_yystr);
			}
		} else
	...
	if (rval == YY_STR) {
		if (ipnat_yysavedepth > 0 && !ipnat_yydictfixed)
			ipnat_yyresetdict();
		if (ipnat_yyexpectaddr != 0)
			ipnat_yyexpectaddr = 0;
	}
	...
	if (ipnat_yydebug)
		printf("lexed(%s) %d,%d,%d [%d,%d,%d] => %d @%d\n",
		       ipnat_yystr, isbuilding, ipnat_yyexpectaddr, ipnat_yysavedepth,
		       string_start, string_end, pos, rval, ipnat_yysavedepth);
...
void ipnat_yysetfixeddict(newdict)
	wordtab_t *newdict;
{
	if (ipnat_yydebug)
		printf("ipnat_yysetfixeddict(%lx)\n", (u_long)newdict);
