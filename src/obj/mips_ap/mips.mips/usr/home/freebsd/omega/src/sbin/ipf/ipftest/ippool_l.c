...
nextchar:
	 ...
	 */
#ifdef	USE_INET6
	if (ippool_yyexpectaddr != 0 && isbuilding == 0 &&
	    (ishex(c) || isdigit(c) || c == ':')) {
...
buildipv6:
...
#ifdef	USE_INET6
		ippool_yystr = ippool_yytexttostr(0, ippool_yypos - 1);
		if (ippool_yystr != NULL) {
			char *s;

			for (s = ippool_yystr; *s && ishex(*s); s++)
				;
			if (!*s && *ippool_yystr) {
				isbuilding = 0;
				c = *ippool_yystr;
	...
	if (ippool_yydebug)
		printf("isbuilding %d ippool_yyvarnext %d nokey %d fixed %d addr %d\n",
		       isbuilding, ippool_yyvarnext, nokey, ippool_yydictfixed, ippool_yyexpectaddr);
	if (isbuilding == 1) {
		wordtab_t *w;
		...
		if ((ippool_yyvarnext == 0) && (nokey == 0)) {
			...
			if (w == NULL && ippool_yywordtab != NULL && !ippool_yydictfixed) {
				ippool_yyresetdict();
				w = ippool_yyfindkey(ippool_yystr);
			}
		} else
	...
	if (rval == YY_STR) {
		if (ippool_yysavedepth > 0 && !ippool_yydictfixed)
			ippool_yyresetdict();
		if (ippool_yyexpectaddr != 0)
			ippool_yyexpectaddr = 0;
	}
	...
	if (ippool_yydebug)
		printf("lexed(%s) %d,%d,%d [%d,%d,%d] => %d @%d\n",
		       ippool_yystr, isbuilding, ippool_yyexpectaddr, ippool_yysavedepth,
		       string_start, string_end, pos, rval, ippool_yysavedepth);
...
void ippool_yysetfixeddict(newdict)
	wordtab_t *newdict;
{
	if (ippool_yydebug)
		printf("ippool_yysetfixeddict(%lx)\n", (u_long)newdict);
